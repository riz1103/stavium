...
export const NoteIcon = ({ duration }: { duration: NoteDuration }) => {
  const base = (duration.startsWith('dotted-') || /^(triplet|quintuplet|sextuplet|septuplet)-/.test(duration))
    ? (duration.replace('dotted-', '').replace(/^(triplet|quintuplet|sextuplet|septuplet)-/, '') as NoteDuration)
    : duration;
...
export const RestIcon = ({ duration }: { duration: NoteDuration }) =>
  REST_ICONS[
    (duration.startsWith('dotted-') || /^(triplet|quintuplet|sextuplet|septuplet)-/.test(duration))
      ? (duration.replace('dotted-', '').replace(/^(triplet|quintuplet|sextuplet|septuplet)-/, '') as NoteDuration)
      : duration
  ] ?? null;
